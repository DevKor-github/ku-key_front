...
interface AlertModalProps extends ModalPortalProps {
...
const AlertModal = (props: AlertModalProps) => {
  const { modalRef, title, content, closeText, confirmText, onConfirm, isOpen, handleClose } = props
  return (
    <ModalPortal isOpen={isOpen} handleClose={handleClose}>
      <ModalCard variant="alert" ref={modalRef}>
        ...
        <div className={css({ display: 'flex', gap: 5 })}>
          {handleClose && <button onClick={handleClose}>{closeText}</button>}
